...
-- step 1: I'll create a table with sequential numbers for each month group in the database
...
-- step 2: I'll check the number of retained customers with a while loop for each consecutive month groups
...
--- buradan asagisi time-gaplerle yapilan bir deneme. yanlis sonuc.
	...
	BEGIN 
			SET @retained_customers = (SELECT COUNT(DISTINCT cust_ID) FROM order_time_gap 
                        WHERE time_gap_between_orders = @counter +1 
                                ...
                                AND time_gap_between_orders =@counter)) 	
...
-- to retrieve it as a table:

declare @retained_cust table
(
  month_id int primary key identity(1,1), 
  retained_customer int
)
DECLARE @counter INT, @max_time_gap INT, @retained_customers INT 
SET @counter = 0 
SET @max_time_gap = (SELECT DATEDIFF(MONTH, MIN(order_date), MAX(Order_Date)) FROM order_time_gap) 
WHILE @counter <= @max_time_gap
	BEGIN 
			...
			INSERT INTO @retained_cust (retained_customer) values (@retained_customers)
			SET @counter +=1
	END
